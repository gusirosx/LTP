module schemes
    ...
    subroutine SOU(ax1,bx1,dpx1,ay1,by1,dpy1,ax2,bx2,dpx2,ay2,by2,dpy2,cx,cy,xww,yss)
        ...
        yss  = - L2
        ax1  =-(4.0*L1 + Lx)
        bx1  = (1.0 + 3.0*L1 + 2.0*Lx)
        dpx1 = (1.0 - 3.0*L2 - 2.0*Ly)
        ay1  =-(4.0*L2 + Ly)
        by1  = (1.0 + 3.0*L2 + 2.0*Ly)
        dpy1 = (1.0 - 3.0*L1 - 2.0*Lx)
        ax2  =-(2.0*L1 + Lx)
    ...
    subroutine QUICK(ax1,bx1,dpx1,ay1,by1,dpy1,ax2,bx2,dpx2,ay2,by2,dpy2,cx,cy,xww,yss)
        ...
        Lx   = dt/(2*dx*dx)
        Ly   = dt/(2*dy*dy)
        L1   = (Pe*dt*cos(alpha))/(16.0*dx)
        L2   = (Pe*dt*sin(alpha))/(16.0*dy)
        cx   = (3.0*L1 - Lx)
        cy   = (3.0*L2 - Ly)  
        xww  = - L1
        yss  = - L2
    ...
    subroutine LOADS_RHS(Ta,RHS_LOADS)
        ...
        do i = 2,N-1
            do j = 2,N-1
                ...
                Zx = Zx + Aux*Piy*(Ta(i+1,j-1) + Ta(i-1,j-1) - 2.0*Ta(i,j-1) - Ta(i+1,j+1) - Ta(i-1,j+1) + 2.0*Ta(i,j+1))
                Zy = Auy*Px*(Ta(i+1,j) - Ta(i-1,j) + Ta(i-1,j-1) - Ta(i+1,j-1))
                Zy = Zy + Auy*Pix*(Ta(i-1,j+1) + Ta(i-1,j-1) - 2.0*Ta(i-1,j) - Ta(i+1,j+1) - Ta(i+1,j-1) + 2.0*Ta(i+1,j))
                RHS_LOADS(i,j) = Zx + Zy
            end do
        end do
        ...
        do i = 2,N-1
            if (i.eq.2) then
                do j = 2,N-1
                    ...
                    Ks = ((3.0*Ta(i,j)  - 4.0*Ta(i+1,j) + Ta(i+2,j))*PIay + (4.0*Ta(i,j)  - 3.0*Ta(i-1,j) - Ta(i+1,j))*PIsy)
                    if (j.eq.2) then
                        Ke = ((Ta(i,j) - Ta(i,j+2))*PIax + (Ta(i,j+1) - Ta(i,j-1))*PIsx)
                        Kw = ((3.0*Ta(i,j) - 4.0*Ta(i,j+1) + Ta(i,j+2))*PIax + (4.0*Ta(i,j) - 3.0*Ta(i,j-1) - Ta(i,j+1))*PIsx)
                    else if (j.gt.2 .and. j.lt.N-1) then
                        Kw = ((Ta(i,j-1) - Ta(i,j+1))*PIax + (Ta(i,j) - Ta(i,j-2))*PIsx)
                        Ke = ((Ta(i,j) - Ta(i,j+2))*PIax + (Ta(i,j+1) - Ta(i,j-1))*PIsx)
                    else if (j.eq.N-1) then
                        Kw = ((Ta(i,j-1) - Ta(i,j+1))*PIax + (Ta(i,j) - Ta(i,j-2))*PIsx)
                        Ke = ((4.0*Ta(i,j) - 3.0*Ta(i,j+1) - Ta(i,j-1))*PIax + (3.0*Ta(i,j) - 4.0*Ta(i,j-1) + Ta(i,j-2))*PIsx)
                    end if
                ...
                do j = 2,N-1
                    Ks = ((Ta(i-1,j) - Ta(i+1,j))*PIay + (Ta(i,j) - Ta(i-2,j))*PIsy)
                    Kn = ((4.0*Ta(i,j) - 3.0*Ta(i+1,j) - Ta(i-1,j))*PIay + (3.0*Ta(i,j) - 4.0*Ta(i-1,j) + Ta(i-2,j))*PIsy)
                    if (j.eq.2) then
                        Ke = ((Ta(i,j) - Ta(i,j+2))*PIax + (Ta(i,j+1) - Ta(i,j-1))*PIsx)
                        Kw = ((3.0*Ta(i,j) - 4.0*Ta(i,j+1) + Ta(i,j+2))*PIax + (4.0*Ta(i,j) - 3.0*Ta(i,j-1) - Ta(i,j+1))*PIsx)
                    else if (j.gt.2 .and. j.lt.N-1) then
